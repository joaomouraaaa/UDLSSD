...
def save_mask(mask, f):
    ...
    # im.save('segmentation_gt_cutpaste/'+f,"JPEG", vmin = 0, vmax = 255)
    im.save('data_transformed/segmentation/'+f,"JPEG", vmin = 0, vmax = 255)

    
def save_image(image, f):
    image = Image.fromarray(np.uint8(image),'L')
    image.save('data_transformed/images/'+f, "JPEG", vmin = 0, vmax = 255)
    
# def save_image_ori(image, f):
#     image = Image.fromarray(np.uint8(image),'L')
#     image.save('save_CEP/'+f+'original', "JPEG", vmin = 0, vmax = 255)
...
files = glob.glob('data_trasnformed/images/*')
for f in files: #cleans folders
    os.remove(f)
files = glob.glob('data_trasnformed/segmentation/*')
for f in files: #cleans folders
    os.remove(f)
...
path_cycle = parent_dir+'/data/segmentation'
